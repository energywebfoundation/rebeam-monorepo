...
import { mockBridgeLocationReturn, mockDbLocationsReturn, formattedLocations } from './utils/spec-data-fixtures';
	...
	describe('fetch location data from the Bridge given a CPO and partyId', () => {
		it('should return locations formatted for the client', async () => {
			...
			jest
				...
				jest
				.spyOn(locationDbService, 'insertLocations')
				.mockResolvedValue();
			const result = await locationService.getCPOLocations({countryCode: "DE", partyId: "CPO"});
