import { Controller, Post, Get, HttpCode, Body, Param, BadGatewayException, UnprocessableEntityException } from '@nestjs/common';
...
export class ChargeController {
	...
	async getChargeSessionData(
		...
		try {
			...
			this.logger.error('Failure to fetch charge session data - check connection to database');
			throw new BadGatewayException(
				new ApiError(
	...
	async getChargeSessionConfirmation(
		...
		try {
			...
			throw new BadGatewayException(
				new ApiError(
				  ApiErrorCode.CHARGE_SESSION,
				  'Failure to fetch charge session confirmation',				  err.message
				)
			  );
		}
	}
