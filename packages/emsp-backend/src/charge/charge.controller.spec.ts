...
import { IBridge, stopBridge, CommandResultType, IOcpiResponse } from '@energyweb/ocn-bridge';
...
describe('ChargeController', () => {
    ...
    describe('stop session', () => {
        ...
        it('should return an Internal Server error if the stop session request fails', async () => {
            ...
            try {
                ...
                expect(status).toBe(HttpStatus.BAD_GATEWAY);
                expect(code).toBe(ApiErrorCode.OCN_BRIDGE);
                expect(message).toBe('The OCN Bridge failed to stop the charging session. Are the desired RPC and OCN Nodes available?');
    ...
    describe('get CDR data', () => {
        it('should fetch and return the formatted cdr data', async () => {
            const mockResultData = { "formattedEndTime": "March 8th, 2022 10:30pm", "formattedCost": "8,00 €", "sessionToken": "c2402e36-0cca-4eb9-b5cd-32eed50ebf63", "id": "c2402e36-0cca-4eb9-b5cd-32eed50ebf63" };
            jest
