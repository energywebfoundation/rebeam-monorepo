...
import { mockCsvData } from './spec-data/session-service-mock-data';
  ...
  describe('get session data', () => {
    it('should send csv data if data is returned from service', async () => {
      ...
      await controller.fetchSessionData(
        new Date('2022-03-06T00:16:01.164Z'),
        new Date('2022-03-07T01:16:01.164Z'),
        res
      );
    ...
    it('should return an Internal Server error if the service fails', async () => {
      jest
        .spyOn(sessionService, 'getSessionFile')
